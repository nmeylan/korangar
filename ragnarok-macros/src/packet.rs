...
pub fn derive_packet_struct(
    ...
    // Collect all header attributes (support multiple for versioned packets)
    ...
    // Use the first header as the default
    let default_signature = packet_signatures[0].signature;

    // Check if this is a versioned packet (multiple headers)
    ...
    if is_versioned {
        ...
        let has_header_field = match &data_struct.fields {
            syn::Fields::Named(fields) => fields.named.iter().any(|f| {
                f.ident.as_ref().map(|ident| ident == "header").unwrap_or(false)
            }),
            _ => false,
        };

        if !has_header_field {
            panic!("Versioned packets (with multiple #[header(...)] attributes) must have a `header: PacketHeader` field with #[hidden_element]");
        }
    }

    // Prepare versions for the helper
    let versions: Vec<(&str, u16)> = if is_versioned {
        // For versioned packets, use the version names (all must have versions at this point)
        packet_signatures
            .iter()
            .map(|sig| (sig.version.as_ref().unwrap().as_str(), sig.signature))
            .collect()
    } else {
        // For non-versioned packets, use "default"
    ...
    // For versioned packets, override packet_header() to use the instance field
