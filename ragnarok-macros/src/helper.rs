...
pub fn byte_convertable_helper(
...
) -> (Vec<TokenStream>, Vec<TokenStream>, Vec<TokenStream>, Vec<TokenStream>, Delimiter) {
    ...
    for (counter, field) in fields.iter_mut().enumerate() {
        ...
        // This is the case for some packet which hold their headers as field, to handle packet id "shuffling".
        // We use #[hidden_element] which is already used by the UI library to hide fields.
    ...
    let new_implementations_vec: Vec<TokenStream> = versions
        ...
        .map(|(version, header_value)| {
            ...
            // Filter out header field from implementations and replace it with the version-specific header
            let filtered_impls: Vec<_> = new_implementations
                ...
                .map(|impl_tokens| {
                    ...
                    if impl_str.starts_with("header") {
                        ...
                        let header_hex = syn::LitInt::new(
                            &format!("{:#x}", header_value),
